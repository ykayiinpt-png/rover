...
class RtcNegotiator(RThread):
    ...
    async def _run(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                await self.handle_websocket_message(message, lock)
    ...
    async def handle_websocket_message(self, message: str, lock: threading.Lock):
        """
    ...
    async def clean(self):
        if self.video_track is not None:
            self.video_track.stop()
            logging.info("[RtcServer] Video track stopped")
